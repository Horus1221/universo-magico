...
function setAuth(v){
  registerMode=v;
  $("tabLogin")?.classList.toggle("active",!v);
  $("tabRegister")?.classList.toggle("active",v);
  $("authSubmit").textContent=v?"CREAR CUENTA":"ENTRAR AL UNIVERSO";
  $("authTitle").textContent=v?"Creá tu aventurero":"Bienvenido al Reino";
}
...
$("authForm")?.addEventListener("submit",async e=>{
  ...
  if(username.length<3||password.length<6){
    showAuth("Nombre mínimo 3 caracteres y contraseña mínima 6.");
    return;
  }
  ...
  try{
    ...
    if(!r.ok){
      throw Error(d.error||"No se pudo entrar.");
    }
    ...
    button.textContent=registerMode?"CREAR CUENTA":"ENTRAR AL UNIVERSO";
  }
});
...
function buildWorld(){
  ...
  makePath(0,0,700,12);
  makePath(-120,20,180,9);
  makePath(130,-70,200,10);
...
function loadPlayer(){
  ...
  loader.load(
    ...
    gltf=>{
      ...
      mixer=
        new THREE.AnimationMixer(player);
    ...
    error=>{
      ...
      toast(
        "No se pudo cargar el personaje 3D."
      );
    }
  );
}
...
function enterHouse(){
  ...
  nearHouse.visible=false;

  nearHouse.userData.inside.visible=true;

  nearHouse.userData.inside.position.copy(
    nearHouse.position
  );

  player.position.set(
    nearHouse.position.x,
    0,
    nearHouse.position.z
  );

  $("enterHouse").hidden=true;
  $("exitHouse").hidden=false;

  toast("Entraste a la casa");
}
...
function exitHouse(){
  ...
  nearHouse.userData.inside.visible=false;
  nearHouse.visible=true;

  player.position.set(
    nearHouse.position.x,
    0,
    nearHouse.position.z+8
  );

  inside=false;

  $("exitHouse").hidden=true;
...
function chooseRace(r){
  ...
  $("abilityPanel").hidden=false;

  $("abilityRace").textContent=
    `${r} — habilidades`;

  $("playerRace").textContent=r;
  ...
  toast(`Raza elegida: ${r}`);
}

function buffSpeed(v,t,msg){
...
function shockwave(msg){
  sparkle(msg);
}

function sparkle(msg){
